...
Pono, and BtorMC. See the notes about witness generation.
    ...
    python modelcheck.py test/smv/Delay.smv btormc --output Delay.smv.witness

    python modelcheck.py test/moxi/Delay.moxi btormc --output Delay.moxi.witness
...
about btormc with kind):
    ...
    python modelcheck.py test/smv/QF_BV/lup.1.prop1-func-interl.btor.smv pono --kind --output lup.1.prop1-func-interl.btor.smv.witness

You can ask the script to copy back all intermediate translation files with the
`--copyback` option:

    python modelcheck.py test/smv/Delay.smv btormc --output Delay.smv.out --copyback
