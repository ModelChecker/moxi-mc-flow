...
from src import log, mcil, json2mcil, btor, parse_mcil

FILE_NAME = Path(__file__).name

mcil_fun_map: dict[str, btor.BtorOperator] = {
...
# A SortMap maps mcil.MCILSorts to BTOR2 sorts
SortMap = dict[mcil.MCILSort, btor.BtorSort]

# An ExprMap maps mcil.MCILExprs to btor.BtorExprs
ExprMap = dict[mcil.MCILExpr, btor.BtorExpr]

BtorVarType = tuple[Optional[btor.BtorStateVar], btor.BtorStateVar, Optional[btor.BtorStateVar]]
...
VarMap = dict[tuple[str, mcil.MCILSystemContext], BtorVarType]


def get_const_vars(context: mcil.MCILContext, var_map: VarMap):
    return {
        btor_cur 
...
def mcilsort2btorsort(
    sort: mcil.MCILSort, context: mcil.MCILContext, sort_map: SortMap
) -> btor.BtorSort:
    if sort in sort_map:
        btor2_sort = sort_map[sort]
    elif mcil.is_bool_sort(sort):
        btor2_sort = btor.BtorBitVec(1)
    elif mcil.is_bitvec_sort(sort):
        btor2_sort = btor.BtorBitVec(sort.identifier.indices[0])
    elif mcil.is_array_sort(sort):
        ...
        raise NotImplementedError(f"mcil.MCIL sort '{sort}' ({type(sort)}) unrecognized")
...
def build_sort_map_expr(
    node: mcil.MCILExpr,
    context: mcil.MCILContext,
    sort_map: SortMap
) -> SortMap:
    """Iteratively recurse `node` and map each `mcil.MCILSort` to a `btor.BtorSort`."""
    for expr in mcil.postorder_mcil(node, context):
...
def build_sort_map_cmd(
    cmd: mcil.MCILCommand, 
    context: mcil.MCILContext,
    sort_map: SortMap
) -> SortMap:
    if isinstance(cmd, mcil.MCILDefineSystem):
    ...
    elif isinstance(cmd, mcil.MCILCheckSystem):
        for expr in cmd.get_exprs():
            build_sort_map_expr(expr, context, sort_map)
    else:
...
def build_var_map_expr(
    ...
    """Iteratively recurse `node` and map each `(mcil.MCILVar, mcil.MCILSystemContext)` pair to a `BtorVarType`. Assumes that the sort of every sub-expression in `node` is present in `sort_map`."""
    for expr in mcil.postorder_mcil(node, context):
        if not (isinstance(expr, mcil.MCILVar) and (expr.symbol, context.system_context) not in var_map):
            continue
        
        var,system_context = context.lookup_var(expr.symbol, context.system_context)

        symbol = mcil.get_scoped_var_symbol(var, system_context)
        ...
        if var in system_context.get_input_symbols():
            var_map[(expr.symbol, context.system_context.copy())] = (
                None,
                btor.BtorStateVar(sort_map[system_context.get_sort(var)], f"{symbol}.cur"),
                btor.BtorStateVar(sort_map[system_context.get_sort(var)], f"{symbol}.next")
            )
        elif var in system_context.get_output_symbols() or var in system_context.get_local_symbols():
            var_map[(expr.symbol, context.system_context.copy())] = (
                btor.BtorStateVar(sort_map[system_context.get_sort(var)], f"{symbol}.init"),
                btor.BtorStateVar(sort_map[system_context.get_sort(var)], f"{symbol}.cur"),
                btor.BtorStateVar(sort_map[system_context.get_sort(var)], f"{symbol}.next")
            )
...
def build_var_map_cmd(
    ...
    """Update `var_map` to map all `mcil.MCILVar` instances to `btor.BtorVar`s"""
    if isinstance(cmd, mcil.MCILCheckSystem):
        ...
        context.pop_system()
    elif isinstance(cmd, mcil.MCILDefineSystem):
...
def build_expr_map(
    ...
    """Builds `expr_map` by mapping each sub-expression of `node` to an equivalent `btor.BtorExpr`. Assumes that the sort of every sub-expression in `node` is present in `sort_map` and every `(mcil.MCILVar, mcil.MCILSystemContext)` pair is present in `var_map`."""
    for expr in mcil.postorder_mcil(node, context):
        if isinstance(expr, mcil.MCILVar) and expr.symbol in context.bound_let_vars:
            expr_map[expr] = expr_map[context.bound_let_vars[expr.symbol]]
        elif isinstance(expr, mcil.MCILVar) and all([b for b in var_map[(expr.symbol, context.system_context)]]):
        ...
        elif isinstance(expr, mcil.MCILVar):
            ...
            expr_map[expr] = btor_var
        elif isinstance(expr, mcil.MCILConstant) and expr.sort.identifier.symbol in context.declared_enum_sorts:
        ...
        elif isinstance(expr, mcil.MCILConstant):
            expr_map[expr] = btor.BtorConst(sort_map[expr.sort], expr.value)
        elif mcil.is_const_array_expr(expr):
            pass
        elif isinstance(expr, mcil.MCILApply) and expr.identifier.symbol in mcil_fun_map:
        ...
        elif isinstance(expr, mcil.MCILLetExpr):
...
def to_btor2_annotations(
    check: mcil.MCILCheckSystem, 
    context: mcil.MCILContext,
    var_map: VarMap,
) -> list[btor.BtorNode]:
    """Returns a list of `btor.BtorNode`s that define annotations for a BTOR2 program that are necessary for the translation of BTOR2 witnesses to mcil.MCIL witnesses.
    ...
    3. Annotates input variables with an `I`, since mcil.MCIL allows for primed inputs in certain spots (i.e., we can't use `btor.BtorInputVar`s in our translation). Only variables of `check` can be inputs.
    4. Annotates Boolean variables with a `B`, mcil.MCIL since distinguishes between Booleans and bit vectors of length 1 and BTOR2 does not.
    """
    ...
    for var_symbol,sys_ctx,cur in [
        ...
        if mcil.is_array_sort(sort):
            index_sort = sort.parameters[0]
            element_sort = sort.parameters[1]
            if mcil.is_array_sort(index_sort) or mcil.is_array_sort(element_sort):
                log.error("Warning: BTOR2 solvers only support arrays of sort signature (Array (_ BitVec X) (_ BitVec Y)). Not adding sort annotation to BTOR2 file.", FILE_NAME)
                continue
        ...
        # Add Bool var symbols. mcil.MCIL distinguishes between Bool and bit vectors of
        # length 1, BTOR2 does not.
        if mcil.is_bool_sort(sort):
            bool_encoding = f"B {cur.with_no_suffix()}"
            sort_comment = btor.BtorNode()
            sort_comment.set_comment_no_space(bool_encoding)
            btor2_annotations.append(sort_comment)

        # Add input var symbols. mcil.MCIL allows for primed inputs in certain spots,
...
def to_btor2_const_arrays(
    ...
    """Returns a list of `btor.BtorNode`s that defines all constant arrays that are present in `context.const_arrays`. BTOR2 supports initializing array-sorted variables with bit-vectors. Each `(as const (Array X Y) val)` mcil.MCIL expression translates to a variable `Array_bv_X_Y_val` where `init(Array_bv_X_Y_val) = val` and `next(Array_bv_X_Y_val) = Array_bv_X_Y_val`."""
...
def to_btor2_var_definitions(
    ...
    Each key/value pair in `var_map` maps a `(mcil.MCILVar, mcil.MCILSystemContext)` to a triple of `btor.BtorStateVar`s, representing the initial, current, and next variants of the system-context-dependent `mcil.MCILVar`. The current variant of each variable is initialized to the initial variant and the next value of the current variant is set to the next variant. For example, if there is an `mcil.MCILVar` "var", that will map to a triple of `btor.BtorStateVar`s `(var.init, var.cur, var.next)` such that `init(var.cur) = var.init` and `next(var.cur) = var.next`.
    """
    ...
    for var_symbol,sys_ctx,(init,cur,next) in [
        ...
        if cur in const_vars:
            ...
            btor2_var_definitions.append(btor.BtorNext(
                    cast(btor.BtorStateVar, cur), 
                    cast(btor.BtorStateVar, cur)
            ))
            continue
        ...
        if sort.symbol in context.declared_enum_sorts:
            ...
            btor2_enum_lt = btor.BtorApply(
                sort_map[mcil.MCIL_BOOL_SORT], btor.BtorOperator.ULTE, (None, None), 
...
def to_btor2_reach_properties(
    ...
    least one bad property. In mcil.MCIL, multiple `:reach` properties asks for a
    ...
    btor_bool_sort = sort_map[mcil.MCIL_BOOL_SORT]
    ...
    for reach_symbol,reach in [
        ...
        flag_next = btor.BtorApply(btor_bool_sort, btor.BtorOperator.ITE, (None, None),
    ...
    if len(flag_vars) == 1:
        ...
        bad_expr = btor.BtorApply(btor_bool_sort, btor.BtorOperator.AND, (None, None), 
...
def translate(
    mcil_program: mcil.MCILProgram, 
    int_width: int
) -> Optional[btor.BtorProgramSet]:
    """Translate `mcil_prog` to an equisatisfiable set of btor.Btor programs, labeled by query symbol. Translates `Int` types to `BitVec`s of width `int_width`. Dumps a pickle file for each generated BTOR2 file if `do_pickle` is true.
    ...
    1. Constructing a mapping of `mcil.MCILSort`s to `btor.BtorSort`s for the target system
    2. Constructing a mapping of `mcil.MCILVar`s to `btor.BtorVar`s for the target system
    ...
    (well_sorted, context) = mcil.sort_check(mcil_program)
    ...
    mcil.inline_funs(mcil_program, context)
    mcil.to_binary_applys(mcil_program, context)
    mcil.to_qfbv(mcil_program, int_width)
...
def translate_file(
    ...
    if input_path.suffix == ".json":
        with open(str(input_path), "r") as file:
            program = json2mcil.from_json(json.load(file))
    ...
    if not btor.write_btor2_program_set(btor2_program_set, output_path, do_overwrite, do_pickle):
        return 1
