...
class Expression(Element):
    ...
    #     """
    #     from . import node
    #     from nuxmv_pyparser import parse_next_expression
...
class MWord(SimpleType):
    ...
    def __init__(self, size, sign=None, *args, **kwargs):
        super(MWord, self).__init__(*args, **kwargs)
        self.size = size
        self.sign = sign
    ...
    def __str__(self):
        string = ((self.sign + " " if self.sign else "") +
                  "word" +"[" + str(self.size) + "]")
        return super(MWord, self).__str__(string=string)

    def __deepcopy__(self, memo):
        return MWord(deepcopy(self.size, memo),
...
class ModuleMetaClass(type):
    ...
    * expression-based sections such as TRANS, INIT, INVAR, FAIRNESS, JUSTICE
    ...
    def _args_internal(mcs, args):
        ...
        from nuxmv_pyparser import (parseAllString, identifier)
        newargs = []
        for arg in args:
            if type(arg) is str:
    ...
    def _section_internal(mcs, section, body):
        ...
        * If `body` is an enumeration, each element is parsed as an expression
        ...
        from nuxmv_pyparser import (parseAllString,
        ...
        if mcs._sections[section][0] == "mapping":
            ...
            if isinstance(body, Mapping):
                ...
                for key, value in body.items():
                    key = ''.join([i for i in key if not i.isdigit()]) # un-gensym
